      ...
      <div class="lg:col-span-2 space-y-8">
        ...
        <div class="bg-white/80 backdrop-blur-md p-4 lg:p-8 rounded-3xl shadow-xl border border-white/20">
          <h2 class="text-2xl font-bold mb-6 flex items-center gap-2">
            <span class="w-1 h-8 bg-gradient-to-b from-blue-600 to-indigo-600 rounded-full block"></span>
            商品清单
            <span class="text-sm font-normal text-gray-500 ml-2">共 {{ cartStore.selectedTotalItems }} 件</span>
          </h2>
          <div class="space-y-6">
            <div v-for="item in cartStore.selectedItems" :key="item.cartItemId || item.id" class="flex gap-4 items-center border-b border-gray-100 last:border-0 pb-6 last:pb-0">
...
async function submitOrder() {
  ...
  orderStore.createOrder(cartStore.selectedItems, form.value, paymentMethod.value)
