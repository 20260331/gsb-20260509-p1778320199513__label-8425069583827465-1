<template>
  <div class="flex items-center justify-center min-h-[calc(100vh-12rem)] py-12 px-4 sm:px-6 lg:px-8">
    <div class="max-w-md w-full space-y-8 bg-white/80 backdrop-blur-md p-10 rounded-3xl shadow-2xl border border-white/20">
      <div>
        <h2 class="mt-2 text-center text-4xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-indigo-600 tracking-tight">创建新账户</h2>
      ...
      <form class="mt-8 space-y-6" @submit.prevent="handleRegister">
        <div class="space-y-5">
          ...
          <div>
            <label for="email" class="block text-sm font-medium text-gray-700 ml-1 mb-1">邮箱</label>
            <div class="mt-1">
              <input id="email" v-model="email" name="email" type="email" required class="appearance-none block w-full px-5 py-3 border border-gray-200 rounded-xl placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all duration-300 bg-gray-50/50" placeholder="请输入邮箱">
            </div>
          </div>
          <div>
        ...
        <div class="text-center">
          <router-link to="/login" class="font-medium text-blue-600 hover:text-indigo-600 transition duration-300 ease-in-out hover:underline underline-offset-4">已有账号？立即登录</router-link>
        </div>
      </form>
    </div>
  </div>
